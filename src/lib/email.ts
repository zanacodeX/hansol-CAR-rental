...
function getResend() {
  ...
  if (!key || key === "re_your_resend_api_key_here") {
    console.warn("[Email] RESEND_API_KEY is not configured - emails will not send");
  }
...
export async function sendBookingRequestEmail(booking: BookingEmailData) {
  ...
  if (!customerEmail) {
    console.warn("[Email] No customer email, skipping booking request email");
    return;
  }
  ...
  try {
    ...
    console.log("[Email] Customer booking request email sent:", result.data?.id);
  } catch (e) {
    console.error("[Email] Failed to send customer booking request email:", e);
  }
  ...
  try {
    ...
    console.error("[Email] Failed to send admin booking request email:", e);
  }
}
...
export async function sendBookingConfirmedEmail(booking: {
  ...
  try {
    ...
    console.log("[Email] Confirmed email sent:", customerEmail);
  } catch (e) {
    console.error("[Email] Failed to send confirmed email:", e);
  }
}
...
export async function sendBookingCancelledEmail(booking: {
  ...
  try {
    ...
    console.log("[Email] Cancelled email sent:", customerEmail);
  } catch (e) {
    console.error("[Email] Failed to send cancelled email:", e);
  }
}
